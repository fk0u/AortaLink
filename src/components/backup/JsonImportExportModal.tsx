...
import { X, Download, Upload, FileJson, CheckCircle2, ShieldCheck, FileUp } from 'lucide-react';
import {
...
export const JsonImportExportModal: React.FC<JsonImportExportModalProps> = ({
  ...
  const processFile = async (file: File) => {
    ...
    try {
      ...
      const res = await restoreAortaLinkJsonPayload(text);
      if (res.success) {
        setCacheDirty(true);
      ...
      addToast({ type: 'error', title: 'Error', message: 'Berkas JSON tidak dapat dibaca.' });
    } finally {
    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-[85] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md overflow-y-auto">
        <motion.div
          ...
          className="bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 rounded-3xl max-w-lg w-full shadow-2xl overflow-hidden my-auto flex flex-col text-slate-900 dark:text-slate-100"
        >
          {/* Header */}
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">
                  Interoperabilitas &amp; Backup Data Rekam Medis (HL7 FHIR R4)
                </p>
              </div>
            </div>
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200/80 dark:border-slate-700/80 space-y-3">
              <div className="flex items-center gap-2 text-xs font-black text-slate-900 dark:text-slate-100">
                ...
                <span>Ekspor Cadangan JSON Kompleks</span>
              </div>
              <p className="text-xs text-slate-500 dark:text-slate-400 leading-relaxed font-medium">
                Unduh seluruh riwayat tensi, regimen obat, parameter lab, dan bundle HL7 FHIR R4 ke dalam 1 berkas JSON terstruktur.
              </p>
              <button
                type="button"
            ...
            <div className="space-y-3">
              ...
              >
                ...
                />
                <label htmlFor="json-file-upload" className="cursor-pointer space-y-2 block">
                  ...
                  <div className="text-xs font-black text-slate-800 dark:text-slate-200">
                    {isImporting ? 'Memproses Berkas...' : 'Klik untuk Memilih Berkas JSON atau Drag & Drop'}
                  </div>
                  <p className="text-[10px] text-slate-400">
                    Format yang didukung: AortaLink Backup JSON v1/v2 &amp; HL7 FHIR R4 Bundle
                  </p>
                </label>
              </div>
            </div>
          ...
          <div className="p-4 bg-slate-50 dark:bg-slate-950 border-t border-slate-100 dark:border-slate-800 text-center text-[10px] text-slate-400 font-medium">
            Validasi Skema Berkas Otomatis • Interoperabilitas Lintas Platform
          </div>
        </motion.div>
      </div>
    </AnimatePresence>
